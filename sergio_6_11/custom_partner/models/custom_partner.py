...
class ResPartner(models.Model):
    ...
    comerciales_asignados_ids = fields.Many2many(
        'res.partner',
        'supervisor_externo_comercial_rel',
        'externo_id', 
        'comercial_id',
    ...
    # NUEVO CAMPO: Clientes asignados a comerciales
    clientes_asignados_ids = fields.Many2many(
        'res.partner',
        'comercial_cliente_rel',
        'comercial_id', 
        'cliente_id',
        string='Clientes Asignados',
        domain="[('worker', '=', False), ('supervisor', '=', False), ('external', '=', False)]",
        help='Clientes asignados a este comercial'
    )
    ...
    # NUEVOS CONSTRAINTS PARA clientes_asignados_ids
    @api.constrains('worker', 'clientes_asignados_ids')
    def _check_clientes_asignados_solo_a_comerciales(self):
        """Validar que solo los comerciales tengan clientes asignados"""
        for rec in self:
            if rec.clientes_asignados_ids and not rec.worker:
                raise ValidationError(
                    "Solo los comerciales pueden tener clientes asignados."
                )

    @api.constrains('worker', 'clientes_asignados_ids')
    def _check_clientes_validos(self):
        """Validar que los clientes asignados sean realmente clientes (sin roles)"""
        for rec in self:
            if rec.worker and rec.clientes_asignados_ids:
                clientes_invalidos = rec.clientes_asignados_ids.filtered(
                    lambda c: c.worker or c.supervisor or c.external
                )
                if clientes_invalidos:
                    raise ValidationError(
                        f"Solo se pueden asignar clientes sin roles. Los siguientes contactos tienen roles: "
                        f"{', '.join(clientes_invalidos.mapped('name'))}"
                    )

    @api.constrains('worker', 'clientes_asignados_ids')
    def _check_clientes_mismo_departamento_empresa(self):
        """Validar que los clientes asignados sean del mismo departamento y empresa"""
        for rec in self:
            if rec.worker and rec.clientes_asignados_ids:
                for cliente in rec.clientes_asignados_ids:
                    if rec.internal_company_id and cliente.internal_company_id:
                        if rec.internal_company_id != cliente.internal_company_id:
                            raise ValidationError(
                                f"El cliente {cliente.name} debe ser de la misma empresa que el comercial.\n"
                                f"Comercial: {rec.internal_company_id.name}\n"
                                f"Cliente: {cliente.internal_company_id.name}"
                            )
                    
                    if rec.department and cliente.department:
                        departamentos_comunes = rec.department & cliente.department
                        if not departamentos_comunes:
                            raise ValidationError(
                                f"El cliente {cliente.name} debe pertenecer al menos a uno de los departamentos del comercial.\n"
                                f"Departamentos del comercial: {rec.department.mapped('name')}\n"
                                f"Departamentos del cliente: {cliente.department.mapped('name')}"
                            )
    ...
    def _onchange_external(self):
        if self.external:
            ...
            self.comerciales_asignados_ids = [(5, 0, 0)]
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            _logger.info(f"   - clientes_asignados_ids: {record.clientes_asignados_ids.ids}")
        ...
        # Si estamos intentando asignar comerciales o clientes, log específico
        if 'comerciales_asignados_ids' in vals:
            _logger.info(f"🎯 INTENTANDO ASIGNAR COMERCIALES: {vals['comerciales_asignados_ids']}")
        if 'clientes_asignados_ids' in vals:
            _logger.info(f"🎯 INTENTANDO ASIGNAR CLIENTES: {vals['clientes_asignados_ids']}")
        ...
        if current_user.partner_id and current_user.partner_id.worker:
            restricted_fields = ['worker', 'supervisor', 'department', 'external', 'supervisor_externo_id', 'supervisores_ids', 'comerciales_asignados_ids', 'clientes_asignados_ids']
    ...
    @api.model
        def search(self, args, offset=0, limit=None, order=None, count=False):
            ...
            # SUPERVISOR (mantener lógica existente)
            if partner.supervisor:
                ...
                # Obtener todos los clientes asignados a comerciales del supervisor
                clientes_de_comerciales = self.env['res.partner'].sudo().search([
                    ('id', 'in', comerciales_ids)
                ]).mapped('clientes_asignados_ids').ids
                
                # Clientes creados por comerciales
                ...
                # Clientes creados por el supervisor
                ...
                # Combinar todos los clientes visibles
                todos_clientes_ids = list(set(
                    clientes_de_comerciales + 
                    clientes_por_comerciales + 
                    clientes_por_supervisor
                ))
            ...
            # EXTERNO (mantener lógica existente)
            ...
            # COMERCIAL - NUEVA LÓGICA CON clientes_asignados_ids
            elif partner.worker:
                _logger.info(f"🔍 COMERCIAL search - Usuario: {partner.name} (ID: {partner.id})")
                
                # Obtener IDs de clientes asignados
                clientes_asignados_ids = partner.clientes_asignados_ids.ids
                
                _logger.info(f"   - Clientes asignados: {len(clientes_asignados_ids)} - IDs: {clientes_asignados_ids}")
                
                worker_domain = [
                    '|',
                    ('id', '=', partner.id),  # Sí mismo
                    ('id', 'in', clientes_asignados_ids)  # Clientes asignados
                ]
        ...
        def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
            ...
            # SUPERVISOR (lógica similar a search)
            if partner.supervisor:
